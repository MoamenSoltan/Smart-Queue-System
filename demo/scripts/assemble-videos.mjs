...
const sceneFiles = {
  patient: path.join(rawDir, "page@eda8155afe55bbb7d267afb9da23ba47.webm"),
  reception: path.join(rawDir, "page@9a163cea9643ea5db488dae2d9da1860.webm"),
  bot: path.join(rawDir, "page@9dfd1257138f6d88edf35478ef6c93ae.webm"),
  admin: path.join(rawDir, "page@cfbaca72d56d21b417940e885b6ab9c9.webm"),
};
...
async function assemble() {
  ...
  run(`ffmpeg -y -i "${sceneFiles.patient}" -i "${lt1}" -i "${lt2}" -i "${lt3}" -f lavfi -t 31.48 -i anullsrc=r=48000:cl=stereo -filter_complex "${filterA}" -map "[v]" -map 4:a -c:v libx264 -preset fast -crf 18 -c:a aac -shortest "${sceneAPath}"`);
  ...
  const filterB = `[0:v]fps=30,scale=1920:1080,format=yuv420p[base];` +
    ...
    `[tmp1][2:v]overlay=enable='between(t,14.5,24)'[v]`;

  run(`ffmpeg -y -i "${sceneFiles.reception}" -i "${lt4}" -i "${lt5}" -f lavfi -t 23.6 -i anullsrc=r=48000:cl=stereo -filter_complex "${filterB}" -map "[v]" -map 3:a -c:v libx264 -preset fast -crf 18 -c:a aac -shortest "${sceneBPath}"`);
  ...
  const filterC = `[0:v]fps=30,scale=1920:1080,format=yuv420p[base];` +
    `[base][1:v]overlay=enable='between(t,0,15)'[v]`;

  run(`ffmpeg -y -i "${sceneFiles.bot}" -i "${lt6}" -f lavfi -t 14.48 -i anullsrc=r=48000:cl=stereo -filter_complex "${filterC}" -map "[v]" -map 2:a -c:v libx264 -preset fast -crf 18 -c:a aac -shortest "${sceneCPath}"`);
  ...
  run(`ffmpeg -y -i "${sceneFiles.admin}" -i "${lt7}" -f lavfi -t 25.68 -i anullsrc=r=48000:cl=stereo -filter_complex "${filterD}" -map "[v]" -map 2:a -c:v libx264 -preset fast -crf 18 -c:a aac -shortest "${sceneDPath}"`);
  ...
  run(`ffmpeg -y -f concat -safe 0 -i "${fullConcatList}" -c copy "${fullDemoOut}"`);
  ...
  run(`ffmpeg -y -ss 0 -to 14 -i "${sceneAPath}" -c copy "${cvA1}"`);
  // Patient Live Tracker & YOUR TURN: 22 to 31.48s (9.5s)
  run(`ffmpeg -y -ss 22 -to 31.48 -i "${sceneAPath}" -c copy "${cvA2}"`);
  // Reception Desk Call Next: 0 to 14.5s
  run(`ffmpeg -y -ss 0 -to 14.5 -i "${sceneBPath}" -c copy "${cvB1}"`);
  // Public Display: 14.5 to 23.6s (9.1s)
  run(`ffmpeg -y -ss 14.5 -to 23.6 -i "${sceneBPath}" -c copy "${cvB2}"`);
  // SmartBot AI: 0 to 10s (10s)
  run(`ffmpeg -y -ss 0 -to 10 -i "${sceneCPath}" -c copy "${cvC}"`);
  // Admin & Analytics: 12 to 24s (12s)
  run(`ffmpeg -y -ss 12 -to 24 -i "${sceneDPath}" -c copy "${cvD}"`);
  ...
  run(`ffmpeg -y -f concat -safe 0 -i "${cvConcatList}" -c copy "${cvDemoOut}"`);
